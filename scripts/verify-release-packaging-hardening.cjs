...
// Local-only config generated by dev/repo-local mode (e.g. configService.ts).
// These are gitignored and must not be treated as archive contaminants when
// validating an extracted archive or no-.git tree, but the git-tracked scan
// above remains strict.
const ARCHIVE_MODE_IGNORED_LOCAL_CONFIG_RE =
...
function isIgnoredGeneratedArchiveModePath(rel) {
  ...
  if (ARCHIVE_MODE_IGNORED_LOCAL_CONFIG_RE.test(rel)) {
    return true;
  }
  return false;
}
