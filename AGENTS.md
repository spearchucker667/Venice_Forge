...
| `VERIFY-068` | Browser traffic containment — normal browsing traffic remains inside the ResearchBrowserView. The `openExternal` API has been replaced by the `liveBrowserAllowExternalOpen` gated `requestOpenInSystemBrowser`. Search tabs and standard flows do not use `target="_blank"` unrestrictedly. Regression-guarded by `verify:browser-traffic-contained`. | `src/components/search/SearchTab.tsx`, `electron/services/researchBrowserServer.ts`, `scripts/verify-browser-traffic-contained.cjs` |
