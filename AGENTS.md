...
**Version:** 2.1.1 | **Stack:** React 19 + TS strict, Electron 42, Express 4, Vitest 4 | **Node:** 22.13+, npm 10+
...
| `VERIFY-060` | Document ingestion XML attribute escaping — every `<attached_file name="...">` wrapper built by `src/services/ingestion/{text,code,pdf,docx,veniceTextParser}Ingestion.ts` runs the file name (and kind/language where applicable) through `escapeXmlAttribute` so a malicious file name cannot close the tag and inject prompt instructions; `src/services/ingestion/xmlEscape.ts` and the ingestion test suites cover the malicious-name case. | `src/services/ingestion/xmlEscape.ts`, `src/services/ingestion/textIngestion.ts`, `src/services/ingestion/codeIngestion.ts`, `src/services/ingestion/pdfIngestion.ts`, `src/services/ingestion/docxIngestion.ts`, `src/services/ingestion/veniceTextParserIngestion.ts`, `src/services/ingestion/*.test.ts` |
