...
- **Atomic config secret redaction:** Plaintext Venice/Jina keys imported from YAML are now removed through an awaited temp-file + rename operation. Initialization no longer reports a successful redaction while the write is still pending, and a failed rename leaves the original YAML intact, removes the temporary file, and surfaces a config initialization error.
...
- **Dead `TABS` constant in `src/constants/venice.ts` (BUG-009, P3):** The legacy `TABS` array (with deprecated ids `batch`, `models`, `gallery`, `diagnostics`) was unreferenced after the canonical tab registry refactor in commit `c6013208`. The constant is now marked `@deprecated` with a pointer to `src/config/tabs.ts`. No runtime behaviour change; this only signals to future contributors that the canonical home is the registry.
- **MiniMax provider abstraction scaffolding (BUG-006, P1):** Added `LlmProvider = "venice" | "minimax"` to `src/config/configSchema.ts` and a `research.llm_provider` field that defaults to `"venice"`. `secrets.minimax_api_key` and `sanitized.secrets.has_minimax_api_key` are added. The env layer (`src/shared/configSchema.ts`) gains `MINIMAX_API_KEY`, `MINIMAX_API_HOST`, `MINIMAX_API_BASE_PATH`, and `DEFAULT_PROVIDER` (parsed through `parseProviderId`, unknown values fall back to `"venice"`). `PROVIDER_CAPABILITIES` and `capabilitiesFor()` are exported from `src/config/configSchema.ts` so the renderer can read the matrix today even though MiniMax is not yet wired. The new env keys are documented in `.env.example`. **No runtime behaviour changes** — every existing call path continues to dispatch to Venice; this is a forward-compat scaffold. Six new tests in `configSchema.test.ts` lock the contract.
- **Streaming parser target update (BUG-007, P2):** The OpenAI-style SSE delta parser in `electron/services/veniceClient.ts:135-149` is the documented target for the future MiniMax wiring. The exact MiniMax streaming format will need a sibling parser; the existing code is left unchanged because the live transport is still Venice. The work is tracked in the new `docs/POST_MINIMAX_M3_AUDIT.md` follow-up section.
...
| VERIFY-020 | Media Studio persistence — image-tools saves a migrated MediaItem to the IDB-backed `useMediaStore` with the correct `operation` (`upscale` / `background-remove` / `edit`) and the new item's image is a data URL (no raw blob leak); extended in the post-MiniMax-M3 audit batch to assert exactly one IDB write per image and zero direct `saveItem` calls. | `src/components/image/image-tools.test.tsx` |
...
| VERIFY-033 | `LlmProvider` / `PROVIDER_CAPABILITIES` / `capabilitiesFor()` defaults to `"venice"` and never leaks the new `minimax_api_key` through the sanitized view. MiniMax is a forward-compat scaffold. | `src/config/configSchema.test.ts` |
