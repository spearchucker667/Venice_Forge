...
export function InspectorPane() {
  ...
  const handleExport = () => {
    ...
    downloadAnchor.setAttribute("download", `venice_forge_traffic_logs_${Date.now()}.json`)
  ...
  return (
    <aside className="w-[480px] border-l border-border bg-surface flex flex-col h-full shrink-0 min-w-0" aria-label="Developer traffic inspector">
      {/* Header */}
      <div className="flex items-center justify-between px-3 h-14 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-semibold text-text-primary">Traffic Inspector</span>
        </div>
        ...
        <div className="w-[180px] border-r border-border overflow-y-auto flex flex-col shrink-0">
          {logs.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-4 text-[12px] text-text-muted text-center">
              ...
              <span className="text-[10px] mt-1 opacity-60">Send chat or generate images to inspect traffic.</span>
            </div>
          ) : (
            logs.map((log) => {
              const isSelected = selectedLog?.id === log.id
              const isBlocked = log.status === 451
              const isError = log.status && (log.status < 200 || log.status >= 300) && log.status !== 451
              return (
                <button
                  key={log.id}
                  ...
                  className={cn(
                    ...
                    isSelected ? "bg-accent/10 text-accent font-medium" : "hover:bg-surface-elevated/40 text-text-secondary"
                  )}
                >
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="font-mono truncate uppercase select-none">{log.method}</span>
                    <span className={cn(
                      "font-mono px-1 rounded-[3px] text-[10px] select-none font-bold",
                      isBlocked ? "bg-warning/20 text-warning" :
                      isError ? "bg-danger/20 text-danger" : "bg-accent/20 text-accent"
                    )}>
                      {log.status || '...'}
                    </span>
                  </div>
                  <span className="text-[12px] font-mono truncate font-semibold select-all" title={log.endpoint}>{log.endpoint}</span>
                  {log.durationMs !== undefined && (
                    <span className="text-[10px] text-text-muted font-mono select-none">{log.durationMs}ms</span>
                  )}
                </button>
              )
            })
          ...
          {selectedLog ? (
            <div className="space-y-4 text-[12px] min-w-0">
              ...
              <div className="p-2 bg-surface-elevated/40 border border-border rounded-md font-mono select-all text-[11px] space-y-0.5">
                <div><span className="text-text-muted font-bold">Time:</span> {new Date(selectedLog.timestamp).toLocaleTimeString()}</div>
                <div><span className="text-text-muted font-bold">URL:</span> {selectedLog.endpoint}</div>
                <div><span className="text-text-muted font-bold">Method:</span> {selectedLog.method}</div>
                <div><span className="text-text-muted font-bold">Status:</span> {selectedLog.status || 'Pending...'}</div>
                {selectedLog.durationMs !== undefined && (
                  <div><span className="text-text-muted font-bold">Latency:</span> {selectedLog.durationMs}ms</div>
                )}
              </div>
              ...
              <div className="space-y-1">
                <span className="font-semibold text-text-secondary select-none">Request Headers</span>
              ...
              <div className="space-y-1">
                <span className="font-semibold text-text-secondary select-none">Request Body</span>
                <pre className="p-2 bg-surface-elevated/40 border border-border rounded-md font-mono text-[10px] overflow-x-auto select-all max-h-48">
                  {selectedLog.requestBody ? JSON.stringify(selectedLog.requestBody, null, 2) : '[Empty]'}
                </pre>
              </div>
              ...
              {selectedLog.error ? (
                ...
                <div className="space-y-1">
                  <span className="font-semibold text-text-secondary select-none">Response Body</span>
                  <pre className="p-2 bg-surface-elevated/40 border border-border rounded-md font-mono text-[10px] overflow-x-auto select-all max-h-60">
                    {selectedLog.responseBody ? JSON.stringify(selectedLog.responseBody, null, 2) : '[Pending or Empty]'}
                  </pre>
                </div>
              )}
            </div>
