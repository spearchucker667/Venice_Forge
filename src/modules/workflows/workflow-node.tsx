...
import { useModels } from '../../hooks/use-models-mock'
...
function ModelSelect({ nodeType, value, onChange }: { nodeType: VeniceNodeType; value: string; onChange: (v: string) => void }) {
  ...
  const options = models?.map((m) => ({ value: m.id, label: (m as any).model_spec?.name || m.id })) ?? [] // eslint-disable-line @typescript-eslint/no-explicit-any
