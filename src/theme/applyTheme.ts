...
export function buildThemeVariableMap(theme: ResolvedTheme): Record<string, string> {
  ...
  return {
    ...
    '--border-hot': (t as unknown as Record<string, string | undefined>).borderHot ?? (theme.mode === 'dark' ? '#743940' : '#d9777f'),
    '--telemetry-cyan': (t as unknown as Record<string, string | undefined>).telemetryCyan ?? '#6ee7d3',
    '--overlay': t.overlay,
    '--glow': t.glow,
