...
import type { VeniceNodeData, VeniceNodeType } from '../../state/workflow-store'
import type { ModelCatalog } from '../../hooks/use-model-catalog-mock'
...
export async function callAgent({ userMessage, draft, history, catalog, model, capabilities, signal, dispatch }: CallAgentOptions): Promise<AgentResponse> {
  ...
  const raw = await singleCall({ model: chosenModel, messages, temperature: 0.3, useResponseFormat: useRF, signal, dispatch })
  ...
  if (parsed.patches.length === 0 && !parsed.say && raw.length > 0) {
    ...
    try {
      const retryRaw = await singleCall({ model: chosenModel, messages: retryMessages, temperature: 0, useResponseFormat: useRF, signal, dispatch })
