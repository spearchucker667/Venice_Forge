...
export const researchBrowserBridge: ResearchBrowserPreloadApi = {
  ...
  requestOpenInSystemBrowser: async (url) =>
    window.veniceForge?.researchBrowser?.requestOpenInSystemBrowser?.(url)
    ?? { ok: false, error: "Unavailable in web mode" },
  setTheme: async (snapshot) =>
    window.veniceForge?.researchBrowser?.setTheme?.(snapshot)
    ?? { ok: false, error: "Unavailable in web mode" },
