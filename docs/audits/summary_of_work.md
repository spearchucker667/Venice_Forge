...
Coverage thresholds are enforced at a 57% branches / 61% functions / 68% lines / 65% statements baseline (Vitest 4 configuration). The long-term project target remains 70% branches / 80% functions+lines+statements.
...
| Command | Result | Date | Notes |
| --- | --- | --- | --- |
| `npm run typecheck` (renderer + Electron main) | PASS | 2026-06-10 | P1-001 server.ts server-authoritative, P2-002 timeout.ts manual AbortController, P2-003 release.yml lint step |
| `npm run lint:eslint` (`--max-warnings=0`) | PASS: 0 warnings | 2026-06-10 | No new warnings; the new describe block in `server.test.ts` is 145 lines and adds no new symbol uses that lint would warn about |
| `npm test` (serial, `--fileParallelism=false`) | PASS: 2154 passed, 1 skipped | 2026-06-10 | +7 net vs 2147 prior baseline (7 new `server.ts Local Family Safe Mode decision matrix` cases; the `timeout.test.ts` change is a strengthening of an existing case, not a new case) |
| `npx vitest run server.test.ts` | PASS: 50/50 | 2026-06-10 | +7 net vs 43 prior baseline: the new "server.ts Local Family Safe Mode decision matrix" describe block pins the full P1-001 behaviour matrix (default ON, header ignored, opt-in honoured, env wins both ways, env=0 disables); the pre-existing "skips the local guard in Adult Mode" safety-middleware test was preserved and updated to set the new opt-in env var |
| `npx vitest run src/utils/timeout.test.ts` | PASS: 7/7 | 2026-06-10 | P2-002 TIMEOUT-CLEANUP-001 (setTimeout/clearTimeout spy confirms `clear()` actually cancelled the internal timer — fails if `createTimeoutSignal` ever uses `AbortSignal.timeout`) and TIMEOUT-CLEANUP-002 (parent abort after `clear()` does not throw) regression guards pass |
| `npx vitest run scripts/verify-archive-clean.test.ts` | PASS: 2/2 | 2026-06-10 | P1-002 INCLUDE_PRIVATE_AUDIT_METADATA gate does not regress the script provenance metadata test |
| `npm run verify:safety-guard` | PASS | 2026-06-10 | Defence-in-depth intact; the new behaviour default-ON is the strictest possible default |
| `npm run verify:markdown-links` | PASS | 2026-06-10 | The new `SWARM_AUDIT_2026_06_09.md` banner adds 17 lines of blockquoted text with no in-doc links |
| `npm run verify:archive-clean` | PASS | 2026-06-10 | `clean-repo-zip.sh` exclude list and `.gitignore` patterns unchanged; the new INCLUDE_PRIVATE_AUDIT_METADATA gate is a runtime env, not a tracked file |
| `npm run verify:release-packaging-hardening` | PASS | 2026-06-10 | No new assertions; existing P1-002 / P1-001 / P1-003 / P1-004 release-pipeline assertions still hold |
| `node scripts/verify-network-boundaries.cjs` | PASS | 2026-06-10 | Jina allowlist assertion still in place; `server.ts` changes are confined to the `isLocalFamilySafeModeEnabled` body |
| `npm run build` | PASS | 2026-06-10 | Renderer + server + Electron outputs |
| `bash scripts/clean-repo-zip.sh ...` + `grep -F "private_audit_metadata=omitted"` on `EXTRACT_INFO.txt` | PASS | 2026-06-10 | Default output records the omit line; with `INCLUDE_PRIVATE_AUDIT_METADATA=1` the four fields are present |
| `grep -F "lint:eslint" .github/workflows/release.yml` | PASS: 3 hits | 2026-06-10 | P2-003: one Lint step per platform job (macos / windows / linux) |
| `grep -F "VENICE_FORGE_ALLOW_CLIENT_SAFETY_OVERRIDE" .env.example` | PASS: 0 hits | 2026-06-10 | P1-001: the env var is intentionally undocumented in `.env.example` so production users do not accidentally enable the renderer-header opt-in |
| `npm audit --omit=dev --audit-level=moderate` | PASS: 0 vulnerabilities | 2026-06-10 | Production dependencies unchanged |
| `npm audit --audit-level=critical` | PASS: 0 vulnerabilities | 2026-06-10 | The 2 critical `concurrently@9.x → shell-quote@1.1.0-1.8.3` were cleared in the 2026-06-09 `concurrently@10.0.3` upgrade |
...
| Command | Result | Date | Notes |
| `npm run typecheck` | PASS: 0 errors | 2026-06-09 | Renderer + Electron main; fixed 4 `unknown` type errors in test file |
| `npm run lint:eslint` | PASS: 0 warnings | 2026-06-09 | `--max-warnings=0` enforced; covers `src electron server.ts scripts` |
| `npm test` (serial) | PASS: 1946 passed, 1 skipped | 2026-06-09 | 182 test files + 1 display-gated smoke (always skipped). All modified surfaces green: veniceSafeMode (10/10), venice-client (12/12), use-chat (11/11). |
