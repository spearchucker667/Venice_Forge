# Seedance Face-Media Consent — Venice API Guide

> Submit face-bearing media to Seedance 2.0 on Venice: the two-call attestation flow, the consent object, the needs_consent response, dedupe, and revocation.
>
> *Source: [docs.venice.ai/guides/media/seedance-face-consent](https://docs.venice.ai/guides/media/seedance-face-consent) — exported 2026-07-11*
...
> **Note:** Consent does not unlock restricted content. A detected **minor combined with sexually-suggestive prompts/NSFW**, or a recognizable **public-figure likeness**, is rejected as a content-policy violation (`422`) and **cannot** be made acceptable by attesting consent.

## The two-call flow

```
POST /video/queue                409 needs_consent          Resubmit same request        200 queue_id
(face media, no consent)  --->   + policy_text       --->   + consents.seedance   --->
```
...
Then poll `POST /api/v1/video/retrieve` with the `queue_id` as usual (see [Video Generation](https://docs.venice.ai/guides/media/video-generation)).
...
> ⚠️ **Warning:** All three fields must be the boolean `true`. Any missing field, a `false`, or any **extra** field — including a `consent_version` — is rejected with a `400`. The policy version is always set by the server; clients never send or pick a version.
...
- Video queue endpoint: [`POST /api/v1/video/queue`](https://docs.venice.ai/api-reference/endpoint/video/queue)
- [Seedance 2.0 Guide](https://docs.venice.ai/guides/media/seedance-2-0) — variants, workflows, prompt syntax, limits
- [Video Generation](https://docs.venice.ai/guides/media/video-generation) — queue / polling overview
