...
- **2026-07-01 React Perf Bug Hunt Fix Session — COMPLETE (current session):** Closed BUG-React #1+#5, #2, #3, #4, #6, #7, #9, #10, #11, #12, #14, #15 across the CORE React, Chat view, and Gallery-view clusters. All twelve HIGH-severity React renderer findings from the earlier audit are now addressed with regression guards, plus the chat-store teardown parity work for #14+#15.
  ...
  - **BUG-React #3:** `message-bubble.tsx` `localSafetyDecision` IIFE replaced with `useMemo([content, localFamilySafeModeEnabled, redTeamMode])` AND gated on `redTeamMode && content && localFamilySafeModeEnabled`. Non-redteam users (the common case) now skip the regex/lookup cost entirely. Three regression tests: redteam-off gate, memo identity, invalidation on content/flag change.
  - **BUG-React #4:** New `src/hooks/useKatexCss.ts` with module-level singleton — `katexCssPromise` is cached at module scope so the dynamic `import('katex/dist/katex.min.css')` happens exactly once for the entire app session. `MessageBubbleImpl` no longer fires the per-bubble useEffect; it calls `useKatexCss()` which just kicks the singleton promise. New regression test (`src/hooks/useKatexCss.test.tsx`) monitors `katexLoadTracker.count` across 50 simultaneous mounts + 50 re-renders; if a future change bypasses the singleton, the count will balloon and fail the test.
  - **BUG-React #6:** `src/components/chat/chat-view.tsx` `availablePriorConversations` now memoized via `useMemo([conversations, activeConversationId])` with comment `// BUG-React#6 regression guard:`. Recomputation on every keystroke eliminated.
  - **BUG-React #7:** `chat-view.tsx` `handleSend`, `handleRemoveFact`, `handleForgetFact` wrapped in `useCallback` with primitive stable deps. `pendingContext` is now mirrored through `pendingContextRef` so async `handleForgetFact` callback (which awaits `askDecision`) always reads the post-render state instead of whatever was captured at callback creation time. Comment: `// BUG-React#7 regression guard: mirror pendingContext through a ref so async handleForgetFact / handleRemoveFact callbacks always see the post-render state, not whatever was captured at callback creation time.`
  - **BUG-React #9:** `src/components/gallery/gallery-view.tsx` initial-load `useEffect(() => { void refresh(); }, [refresh])` (was `[]`) — `refresh` added to dep list to satisfy `react-hooks/exhaustive-deps` so a future store refactor cannot silently swap the closure. Comment: `// BUG-React#9 regression guard:`.
  - **BUG-React #10:** Two separate dep-on-`filtered` effects (originally calling `setVisibleMediaIds(filtered.map(...))` and `reconcileWithVisible(filtered.map(...))`) merged into one memoized `useEffect` — `filteredIds = useMemo([filtered])` then one `useEffect([filteredIds])` calls both store mutations. `filteredRef` source type narrowed from `MediaItem[]` to `string[]` to match the new id-array shape. Registration body line 167 fixed: `visibleIds: () => filteredRef.current` (no obsolete `.map((i) => i.id)`). Comment: `// BUG-React#10 regression guard: combine setVisibleMediaIds + reconcileWithVisible into a single scheduled effect on the memoized id list so subscribers only re-render once per filter change.`
  - **BUG-React #11:** `gallery-view.tsx` children-detection effect — `missingChildIds` is now mirrored through `missingChildIdsRef = useRef(...); useEffect(() => { missingChildIdsRef.current = missingChildIds }, [missingChildIds])` and the detection body reads `!missingChildIdsRef.current.includes(id)`. Dep array dropped from `[inspectorItem, items, loadById, missingChildIds]` to `[inspectorItem, items, loadById]` so the no-op re-run that `Array.from(new Set(...))` causes every setState cycle is eliminated. Comment: `// BUG-React#11 regression guard:`.
  - **BUG-React #12:** `gallery-view.tsx` `registerMediaCommandHandlers` effect (which runs with `[]` deps) was capturing stale `runExport`/`runBulkAddTag` identities. Added `runExportRef` + `runBulkAddTagRef` declared near the selectedMediaIds selector, and a forwarder useEffect `[runExport, runBulkAddTag]` that mutates them after the useCallback declarations. Registration body now uses `runExportRef.current?.(ids)` and `runBulkAddTagRef.current?.(ids, [tag])`. Comment: `// BUG-React#12 regression guard: captive closures inside the empty-deps registration effect need the latest stable runExport/runBulkAddTag identities; ref mirrors forward them through a useEffect that has those callbacks in its dep array.`
...
- **2026-07-01 Three-Area Bug Hunt Fix Session (current session):**
  ...
  - **Fix #2 (HIGH Bug 2.1):** `electron/ipc/validation.ts` `validateVeniceIpcRequest.headers` extended blocklist — added `BLOCKED_VENICE_HEADERS` Set (authorization, host, cookie, content-length, transfer-encoding, origin, referer, proxy-authorization, proxy-authenticate) and `lower.startsWith("x-forwarded-")` prefix check at lines 121-123.
  - **Fix #3 (HIGH Bugs 5.3/5.7):** `src/services/inspectorTelemetry.ts` `SENSITIVE_HEADERS` Set extended with `proxy-authorization` and `proxy-authenticate`.
  - **Fix #4 (HIGH Bug 5.4):** `src/services/inspectorTelemetry.ts` `sanitizeMessage` now redacts `name` field through `summarizeString` (or `[redacted message name]` placeholder for non-string values).
  - **Fix #5 (HIGH Bug 5.9):** `src/services/inspectorTelemetry.ts` `sanitizeInspectorResponse` always summarizes `dataUrl`, `imageUrl`, `url`, `audioUrl`, `videoUrl`, `downloadUrl` via `URL_RESPONSE_FIELDS` Set, regardless of whether values look like data URLs (preserves other fields via `{ ...redactSecrets(record) }`).
  - **Fix #6 (HIGH Bug 3.1):** `src/research/providers/genericHttpScrapeProvider.ts` `::` / `::ffff:` handler extended to also catch the IPv4-compatible IPv6 shortform (`::<private-v4>` dotted quad AND compressed hex `::c0a8:101` / `::7f00:1` / `::a00:1`) which previously bypassed the IPv4 CIDR check because the `mappedV4` derivation only matched the `::ffff:` prefix branch. Regression test added covering all three forms.
  - **Fix #7 (MEDIUM Bug 7.1):** `electron/services/bridgeServer.ts` per-endpoint rate limiting wired at the top of `app.all("*")` handler — pure `checkBridgeRateLimit()` keyed on `(method,path,bucket)` with `BRIDGE_STRICT_LIMIT_PER_MIN=30` for POST and `BRIDGE_RELAXED_LIMIT_PER_MIN=120` for GET, `/ping` exemption, 429 response with `Retry-After` header.
  - **Files changed:** `electron/ipc/handlers/systemHandlers.ts`, `electron/ipc/validation.ts`, `src/services/inspectorTelemetry.ts`, `src/research/providers/genericHttpScrapeProvider.ts`, `src/research/providers/genericHttpScrapeProvider.test.ts`, `electron/services/bridgeServer.ts`, `electron/services/bridgeServer.test.ts`.
  - **Validation:** `npm run lint:eslint` PASS; `npm run typecheck` PASS; `npx vitest run electron/ipc/handlers.test.ts electron/ipc/validation.test.ts src/services/inspectorTelemetry.test.ts electron/services/bridgeServer.test.ts src/research/providers/genericHttpScrapeProvider.test.ts --fileParallelism=false` PASS (`handlers` 32 + `validation` 14 + `inspector` 7 + `bridgeServer` 28 + `genericHttpScrapeProvider` 28 = 109 tests); `npm run verify:safety-guard` PASS; `npm run verify:network-boundaries` PASS; `git diff --check` PASS.
  - **Open TODO updates:** Bug-hunt React findings (MessageBubble memo, useCharacterImage deps, etc.) explicitly recorded as deferred P2 perf-quality work distinct from the security fixes just applied. **Status note: 2026-07-01 React perf session subsequently closed twelve HIGH-severity React items (#1+#5, #2, #3, #4, #6+#7, #9, #10, #11, #12, #14, #15) with regression guards; the five remaining MEDIUM/LOW items (#13, #17, #18, #19, #20) are still deferred.**
