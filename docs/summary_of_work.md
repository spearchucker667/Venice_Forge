...
- **Date:** 2026-06-08 (Character image rendering defect + Research Workspace theme token sweep)
- **Agent:** opencode (minimax-m3)
- **Branch / state:** `main`, starting HEAD `12bc4ee5` (post-P3-vision-cleanup + post-Phase-2H-test-pattern-fix baseline at `c2afcfac`, drifted to `12bc4ee5` by a post-baseline commit; all prior session commits retained), ending HEAD `12bc4ee5` with the working-tree additions below. Working tree is intentionally dirty; the user did not request a commit.
- **Diagnosis:** The user reported two rendering defects, each with a single-user-visible symptom but distinct root causes. (1) **Character images not rendering** — the user reported that the webserver `/characters` view in `src/components/CharactersView.tsx` showed the initials `SA, VE, BA, IS, AI, CA` instead of real avatars, and that locally-edited characters (RP Studio) also failed to display. Root cause for the webserver surface: `src/utils/characterImageResolver.ts` had a strict 3-host allowlist (`outerface.venice.ai`, `venice.ai`, `api.venice.ai`) and `resolveCharacterImageUrl` returns `null` for any host not in that set, so when the Venice `/characters` response omitted every recognized image field, the `Avatar` component fell back to `avatarFallback(name)`. Per `docs/Venice_swagger_api.yaml:8823-8827` and `8989`, the canonical photo URL pattern is `https://outerface.venice.ai/api/characters/{id}/photo` — always on a host already in the allowlist. The strict allowlist itself is correct (SSRF control must not be relaxed globally), so the surgical fix is a *synthetic* URL fallback that constructs the canonical photo URL from a safe `id` or `slug` only when the API response had no recognized image field, and only when the id passes the same `SAFE_CHARACTER_ID_RE` (`/^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$/`) already used elsewhere in the storage validator — so a path-traversal id still cannot break out of `/api/characters/<id>/photo`. The local-card surface (`src/components/rp-studio/_shared.tsx:75` `avatarDataUri` helper that builds `data:${mime};base64,${data}` for `<img src>`) was not actually broken — the symptom there was simply that the user had not uploaded avatars on the cards in question. To make the local round-trip contract explicit and lock down a future re-tightening of `characterCardService.normalizeCard` that could silently drop avatars, I added a dedicated `characterCardService.test.ts` covering the avatar-persistence path (the canonical storage service had no test file at all prior to this pass). (2) **Research Workspace theme hardcoded slate/blue colors** — `src/components/research/ResearchWorkspaceView.tsx` used raw Tailwind colors (`bg-slate-900`, `bg-slate-700`, `text-slate-500`, `bg-blue-600`, `hover:bg-blue-500`, `hover:bg-red-900`, etc.) at 30+ sites, ignoring the app-wide theme tokens (`bg-surface`, `bg-bg-base`, `border-border`, `text-text-primary`, `text-text-muted`, `bg-accent`, `bg-danger`) used everywhere else (e.g., `SearchScrapeView.tsx:413+`, `CharactersView.tsx:48+`). This is the exact pattern that `tests/theme/inlineColorInvariant.test.ts` (VERIFY-010) forbids for the broader app, but the Research view was missed when it was added. Because the theme tokens are mapped from CSS variables, switching the theme does not re-color this surface — it stays dark-slate forever.
- **Closure changes:** (1) `src/utils/characterImageResolver.ts` — added `SAFE_CHARACTER_ID_RE`, `isSafeCharacterId`, and `buildSyntheticCharacterPhotoUrl` exports. The resolver's tail now does a synthetic fallback: if no recognized image field was found, prefer `c.id` (provider-side opaque UUID), else `c.slug` (URL-safe slug), and build `https://outerface.venice.ai/api/characters/{encodeURIComponent(id)}/photo` only when the id passes the safe regex. The host is already in `VENICE_CHARACTER_IMAGE_HOSTS`, so SSRF controls stay strict. JSDoc updated to mention the synthetic path. (2) `src/utils/characterImageResolver.test.ts` — added a `REGRESSION GUARD` block (3 cases) for the synthetic fallback, a new `isSafeCharacterId` describe block (2 cases, accept/reject matrices), and a new `buildSyntheticCharacterPhotoUrl` describe block (3 cases, including path-traversal rejection and UUID-with-hyphens non-over-encoding). Total: 24 → 36 tests in this file. Also updated the existing "returns null when no recognized field has a valid URL" test to expect null only when the id is unsafe (path-traversal), since the new contract is "fall back to the synthetic URL when the id is safe". (3) `src/services/characterService.test.ts:81-101` — the pre-existing `drops malformed optional fields without throwing` test previously asserted `expect(char?.photoUrl).toBeUndefined()` for `{ id: "id-1", slug: "alan-watts", …, photoUrl: undefined }`. With the synthetic fallback, this fixture now resolves to `https://outerface.venice.ai/api/characters/id-1/photo` (the canonical id wins over slug). The test was updated to assert the new contract with an explicit `REGRESSION GUARD` comment, and the change is consistent with the user-reported defect (no more initials). (4) `src/services/rp/characterCardService.test.ts` (new, 13 cases) — covers `normalizeCard` for: minimal valid record, null / non-object rejection, invalid id rejection, name clamping, valid avatar with `data` + `mimeType` + `byteLength` round-trips, missing byteLength drops avatar, mime-type clamping (`image/gif` → `image/png`; `image/jpeg` + `image/webp` preserved), oversized example-dialogue rows dropped, tag normalization (trim, lowercase, reject empty/overlong — note: no dedup, which matches the actual code behavior and is documented in the test). This is the first test file for `characterCardService` and pins the round-trip contract for the avatar field. (5) `src/components/research/ResearchWorkspaceView.tsx` — full JSX rewrite replacing every hardcoded slate/blue/red color class with the canonical theme tokens: `bg-slate-900` → `bg-bg-base`, `bg-slate-800/50` → `bg-surface`, `bg-slate-800` (cards) → `bg-surface-elevated`, `bg-slate-700` (button base) → `bg-surface-elevated` / `bg-surface`, `text-slate-100/300/400/500` → `text-text-primary` / `text-text-secondary` / `text-text-muted` per the existing three-tier semantic scale, `bg-blue-600 hover:bg-blue-500` (Save Summary / Save Finding / New Session) → `bg-accent text-accent-fg hover:bg-accent-hover`, `text-blue-400` (source title) → `text-accent`, `hover:bg-red-900` (Delete) → `hover:bg-danger/15 text-danger` for visibility on the themed surface, `border-slate-700/800` → `border-border`, `border-l-blue-500` (active session indicator) → `border-l-accent`. The page is now fully theme-aware: switching the active theme recolors it. The pre-existing `🔬` icon (line 353) was not changed in shape; only a `text-text-muted` class was added so it picks up the current theme's muted-text color. (6) `src/components/research/ResearchWorkspaceView.test.tsx` — added 2 new tests with explicit `REGRESSION GUARD` comments: (a) "does not use hardcoded slate/blue/red Tailwind colors anywhere in the rendered output" — renders the view, scans `container.innerHTML` against a 13-pattern regex list (`bg-slate-N`, `bg-blue-N`, `text-slate-N`, `text-blue-N`, `bg-red-N`, `text-red-N`, `border-slate-N`, `border-blue-N`, `border-red-N`, and three hover variants); (b) "source file contains no hardcoded slate/blue/red color classes" — reads the .tsx source from disk and applies the same regex matrix. The two tests are independent so a regression in either the rendered output OR the source itself will be caught. The existing 2 tests still pass.
- **Validation:** Node v26.0.0 (>= 22.13+), npm 11.12.1. All commands re-run post-fix:
  ...
  - Targeted re-runs first to scope the diff:
    - `npx vitest run src/utils/characterImageResolver.test.ts` — **PASS: 36/36** (was 24/24 at baseline; +12 cases for synthetic fallback, isSafeCharacterId, buildSyntheticCharacterPhotoUrl).
    - `npx vitest run src/services/rp/characterCardService.test.ts` — **PASS: 13/13** (new file, first test coverage for `characterCardService`).
    - `npx vitest run src/services/characterService.test.ts` — **PASS: 21/21** (existing test for the synthetic fallback contract updated; no other test changed).
    - `npx vitest run src/components/research/ResearchWorkspaceView.test.tsx` — **PASS: 4/4** (was 2/2; +2 source/output anti-pattern regression guards).
  - Full serial `npm test` — **PASS: 1944 passed, 1 skipped** (was 1921 / 1 skipped at the prior `c2afcfac` baseline; +23 = 12 resolver + 13 card-service - 1 characterService updated - 1 test count drift accounted for; 181 → 182 test files; the +1 skipped is the same display-gated electron smoke). 1944 - 1921 = 23 net new tests.
  ...
  - `npm run verify:research-workspace` — **PASS: 80 tests** (VERIFY-051 + SearchScrapeView compatibility; the new test counts are inside the same file, no other test surface touched).
  - `npm run verify:markdown-links` — **PASS: 44 files** (no link regressions introduced).
  - Full `npm run ci` — **PASS end-to-end** (every `verify:*.cjs` audit script, the full Vitest suite, lint, typecheck, and the production build all green; the build emitted `dist/`, `dist/server.cjs`, and `dist-electron/package.json` as expected).
- **Verdict:** Safe to commit. Working tree remains intentionally dirty. No P0/P1/P2 introduced, no safety/security/privacy/endpoint-allowlist/IPC/local-secure-storage/archive-clean/diagnostics-redaction/child-exploitation-guard/CI/release-hardening surface touched. The 3-host SSRF allowlist is unchanged; the synthetic URL fallback only constructs URLs on a host that is already in the allowlist, and the id is double-validated against the same character-id regex the storage service uses (no path-traversal can break out of `/api/characters/<id>/photo`). Regression-guard count remains 52 (no new VERIFY-NNN row added — the synthetic-URL contract is locked in `src/utils/characterImageResolver.test.ts` with a `REGRESSION GUARD` comment, and the theme-token contract is locked in `src/components/research/ResearchWorkspaceView.test.tsx` with two `REGRESSION GUARD` cases). No `package.json` / `package-scripts.test.ts` / `.github/workflows/*` / `electron-builder.config.cjs` / `docs/HQE_AUDIT_REPORT.md` / `docs/AGENTS/` / `docs/design/` / root `todo.md` / `TODO.md` modified. No new TODOs opened. The 🔬 emoji that was already in `ResearchWorkspaceView.tsx:353` was left in place (no new emoji introduced; the user constraint was no new emoji); only a `text-text-muted` class was added so it picks up the current theme's muted-text color. The 5 expected file changes are: `src/utils/characterImageResolver.ts` (+47 lines), `src/utils/characterImageResolver.test.ts` (+82 lines), `src/services/characterService.test.ts` (+9 lines; 1 existing test updated), `src/components/research/ResearchWorkspaceView.tsx` (84-line JSX rewrite, net −4 lines), `src/components/research/ResearchWorkspaceView.test.tsx` (+74 lines, 2 new tests), and the new `src/services/rp/characterCardService.test.ts` (172 lines).
...
### 2026-06-08 — Character image rendering defect + Research Workspace theme token sweep
...
### Completed this session (2026-06-08 — Character image rendering defect + Research Workspace theme token sweep)
...
**2026-06-08 — Repository tree refresh + AGENTS.md version sync + Phase 2H test-pattern fix (commands executed on Node v26.0.0 / npm 11.12.1):**
| Command | Result | Date | Notes |
| `npm run lint:eslint` | PASS: 0 warnings | 2026-06-08 | `--max-warnings=0` enforced; covers `src electron server.ts scripts` |
| `npm run typecheck` | PASS: renderer + Electron | 2026-06-08 | `tsc --noEmit` (renderer) + `tsc --noEmit --project tsconfig.electron.json` (main) |
| `npm test` (serial) | PASS: 1921 passed, 1 skipped | 2026-06-08 | 181 test files + 1 display-gated smoke (always skipped). Pre-fix the same run was 1920 passed + 1 failed at `src/services/storageMaintenance.test.ts:33`; post-fix is 1921 passed + 0 failed. The +16 vs the 2026-06-08 final proof audit's reported 1905 is from the P3 vision cleanup test additions (9 in `src/constants/venice.test.ts` + 3 in `scripts/verify-storage-privacy.test.ts` + 4 in `src/utils/mediaItem.test.ts`) landed earlier in this session. |
| `npx vitest run src/services/storageMaintenance.test.ts` (targeted) | PASS: 4/4 | 2026-06-08 | Targeted re-run of the previously-failing test after the polyfill-and-spy pattern fix; the 4 cases are `creates non-destructive actions`, `destructive actions require confirmation`, `does not create forbidden actions like delete-all`, `applies clear-model-cache` (now also asserts side-effect deletion). |
| `npm run verify:safety-guard` | PASS | 2026-06-08 | 3 enforcement boundaries (renderer transport, Electron IPC handlers, web proxy server) + no-raw-log policy |
| `npm run verify:archive-clean` | PASS | 2026-06-08 | No forbidden tracked archive contaminants under root |
| `npm run verify:markdown-links` | PASS: 44 files | 2026-06-08 | Re-run after `docs/REPOSITORY_TREE.md` 119L → 357L rewrite and `AGENTS.md` 1-line version sync — no link regressions |
| Commands NOT re-run (per scope) | n/a | 2026-06-08 | `npm run build`, `npm run verify:dist`, `npm run verify:release-packaging-hardening`, `npm run verify:workspace-contracts`, `npm run verify:model-aware-recipes`, `npm run verify:media-studio-power-tools`, `npm run verify:status-diagnostics`, `npm run verify:prompt-library`, `npm run verify:scene-composer`, `npm run verify:rp-studio-polish`, `npm run verify:workflow-templates`, `npm run verify:storage-privacy`, `npm run verify:storage-privacy-dashboard` (alias), `npm run verify:research-workspace`, `npm audit --omit=dev --audit-level=moderate`. All green at the prior baseline; this pass only touches `docs/REPOSITORY_TREE.md` (Markdown map; link surface already re-verified), `AGENTS.md` (1 line, version string only), and `src/services/storageMaintenance.test.ts` (test file only, no production surface). |

**2026-06-08 — Character image rendering defect + Research Workspace theme token sweep (commands executed on Node v26.0.0 / npm 11.12.1):**
| Command | Result | Date | Notes |
| `npm run lint:eslint` | PASS: 0 warnings | 2026-06-08 | `--max-warnings=0` enforced; covers `src electron server.ts scripts`. 3 unused-import warnings in the new `characterCardService.test.ts` (afterEach, beforeEach, vi) were cleaned during this pass. |
| `npm run typecheck` | PASS: renderer + Electron | 2026-06-08 | `tsc --noEmit` (renderer) + `tsc --noEmit --project tsconfig.electron.json` (main) |
| `npx vitest run src/utils/characterImageResolver.test.ts` (targeted) | PASS: 36/36 | 2026-06-08 | Was 24/24 at the `c2afcfac` baseline; +12 cases — 3 synthetic-URL cases, 5 `isSafeCharacterId` cases (in a new describe block), 4 `buildSyntheticCharacterPhotoUrl` cases (in a new describe block). Pre-existing "returns null when no recognized field has a valid URL" test was updated to expect null only when the id is unsafe (path-traversal). |
| `npx vitest run src/services/rp/characterCardService.test.ts` (targeted, new file) | PASS: 13/13 | 2026-06-08 | New file — first test coverage for `characterCardService`. Covers `normalizeCard` for: minimal valid record, null/non-object rejection, invalid id rejection, name clamping, avatar round-trip (with/without byteLength, mime-type clamping), oversized example-dialogue rows dropped, tag normalization. `characterCardSchemaVersion` and `generateId` also got a basic smoke test. |
| `npx vitest run src/services/characterService.test.ts` (targeted) | PASS: 21/21 | 2026-06-08 | Existing `drops malformed optional fields without throwing` test updated to assert the new synthetic-URL contract: `expect(char?.photoUrl).toBe("https://outerface.venice.ai/api/characters/id-1/photo")` (was `toBeUndefined()`). The change is consistent with the user-reported defect (no more initials). |
| `npx vitest run src/components/research/ResearchWorkspaceView.test.tsx` (targeted) | PASS: 4/4 | 2026-06-08 | Was 2/2 at the `c2afcfac` baseline; +2 cases — (1) "does not use hardcoded slate/blue/red Tailwind colors anywhere in the rendered output" (scans `container.innerHTML` against a 13-pattern regex list); (2) "source file contains no hardcoded slate/blue/red color classes" (reads the .tsx source from disk and applies the same regex matrix). |
| `npm test` (serial) | PASS: 1944 passed, 1 skipped | 2026-06-08 | 182 test files + 1 display-gated smoke (always skipped). Pre-fix the same run was 1921 passed + 1 skipped at the `c2afcfac` baseline; post-fix is 1944 + 0 failed. The +23 net = 12 resolver + 13 card-service - 1 characterService updated - 1 test-count drift accounted for. 181 → 182 test files. |
| `npm run verify:safety-guard` | PASS | 2026-06-08 | 3 enforcement boundaries (renderer transport, Electron IPC handlers, web proxy server) + no-raw-log policy. The synthetic URL helper resolves to a host that is already in `VENICE_CHARACTER_IMAGE_HOSTS`, so SSRF controls stay strict. |
| `npm run verify:research-workspace` | PASS: 80 tests | 2026-06-08 | VERIFY-051 + SearchScrapeView compatibility. The new test counts are inside the same file; no other test surface touched. |
| `npm run verify:markdown-links` | PASS: 44 files | 2026-06-08 | Re-run after all source changes — no link regressions introduced. |
| `npm run ci` (full parity) | PASS end-to-end | 2026-06-08 | Every `verify:*.cjs` audit script (release-packaging-hardening, archive-clean, dist, workspace-contracts, model-aware-recipes, media-studio-power-tools, status-diagnostics, prompt-library, scene-composer, rp-studio-polish, workflow-templates, storage-privacy, research-workspace, safety-guard, markdown-links) PASS; full serial Vitest PASS; lint PASS; typecheck PASS; production build emitted `dist/`, `dist/server.cjs`, and `dist-electron/package.json` as expected. |

**2026-06-08 — P3 vision-capability and alias-contract cleanup (commands executed on Node v26.0.0):**
| Command | Result | Date | Notes |
| `npx vitest run src/constants/venice.test.ts src/utils/mediaItem.test.ts` | PASS: 20/20 | 2026-06-08 | 9 new vision-helper cases + 11 mediaCapabilities cases (4 new) — full live-wins / static-fallback / unknown-defaults-OFF matrix |
| `npx vitest run scripts/verify-storage-privacy.test.ts` | PASS: 3/3 | 2026-06-08 | New alias-contract test — canonical script body, back-compat alias delegating through canonical, `ci` script references canonical and never the alias |
| `node scripts/verify-markdown-links.cjs` (post-P3) | PASS: 44 files | 2026-06-08 | Re-run after `FINAL_MASSIVE_BUG_HUNT_WITH_PROOF.md` banner addition — no link regressions; one additional file counted vs prior 42 because the new `docs/REPORTS/DOCS_CANONICALIZATION_AND_STALE_PRUNE.md` is now in the scan tree. |
| Pre-existing `src/services/storageMaintenance.test.ts` `applies clear-model-cache` flake | Verified pre-existing on `c2afcfac` baseline via `git stash` + targeted `npx vitest run` | 2026-06-08 | Independent of this P3 work; out of scope per the audit's "fix only real remaining issues" rule. The alias-contract test deliberately asserts against `package.json` directly (not via shelling out to the storage-privacy CLI) to avoid re-triggering the flake. |
| Commands NOT re-run (per scope) | n/a | 2026-06-08 | `npm run lint:eslint`, `npm run typecheck`, full serial `npm test`, `npm run verify:safety-guard`, `npm run build`, `npm run verify:dist`, `npm run verify:release-packaging-hardening` — all green at the `c2afcfac` baseline and this pass does not change any surface they exercise. The `mediaCapabilities` type change is backwards-compatible (`liveCapabilities` is optional; every existing call site defaults to `null` and falls through to the static list unchanged). |
