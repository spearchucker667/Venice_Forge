...
* **VF-VENICE-API-2026-09-16-FEATURE-GAP-P1-TRANCHE** — Phase 2 (Billing/Usage: 3 GET-only endpoints opened, deprecated `/billing/usage` intentionally absent), Phase 4 (E2EE: capability-gated `enable_e2ee`, provider-fallback strip), and Phase 5.5 (TTS per-model `supported_formats` + `default_format` resolution) implemented and locally validated against `main` baseline `199a68c6`. **Sub-deliberately deferred within the tranche**: Phase 5.1 (`bitrate_mode`), 5.2 (source-match Seedance options), 5.3 (20k prompt length — needs per-route confirmation), 5.4 (input-image pricing — needs `VideoQuoteWirePayload` audit). Each needs upstream Swagger evidence per handoff §31. **Deferred to subsequent sessions**: Phases 6 (file/video inputs), 7 (cache retention), 8 (Responses API alpha — Do-Not-Rule: do not silently switch chat→responses), 9 (API-key CRUD), 10 (x402 wallet auth — Do-Not-Rule: do not auto-top-up), 11 (Crypto RPC — Do-Not-Rule: do not allow arbitrary crypto network/path strings), 12 (character reviews), 13 (optional investigations). Publication: commit and push to `main`, then verify hosted CI/CodeQL green against the pushed SHA.

* **VF-VENICE-API-2026-09-16-FEATURE-GAP-FIRST-SLICE** — Phase 0 (contract sync), Phase 1 (`model_spec.uncensored` precedence, caller/profile-scoped `/models`, caller-specific cost vs list pricing), and Phase 3 (typed `VeniceRateLimitReason` + `Retry-After` preservation + UX mapper) implemented and locally validated against `main` baseline `19c820ff`. Publication: committed (`c395384f` + `199a68c6`), hosted CI/CodeQL green against `199a68c6`. Remaining phases from `docs/audits/TODO/VENICE_API_2026-09-16_FEATURE_GAP_AGENT_HANDOFF.md` are sequenced for future sessions: Phase 2 (Billing/Usage), Phase 4 (E2EE), Phase 5 (Seedance/media delta), Phases 6–10 (file/video inputs, cache retention, Responses API, API-key admin, x402 wallet auth), and Phases 11–13 (Crypto RPC, character reviews, optional investigations). Per handoff §31, do not mechanically add the remaining endpoints without per-phase contract verification.
