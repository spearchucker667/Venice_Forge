...
- **Source-of-truth reconciliation (correction to prior review):**
  - The prior review claimed the local api-docs checkout was the source of truth and was at `db3b9f4f...`. This was incorrect. That checkout is the user's own fork (`origin = https://github.com/spearchucker667/api-docs.git`), which is stale at `db3b9f4f` (2026-08-14).
...
- **Out of scope (deferred to next session if user requests):**
  ...
  - Updating the local api-docs fork (`spearchucker667/api-docs`) to current upstream — not authorized.
- **Risks/notes:**
