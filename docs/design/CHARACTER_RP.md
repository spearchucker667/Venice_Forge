...
| `sceneGenerationService` | `extractScenePrompt(chat, opts)` and `generateScene(chat, req)`. **Step 1: `assessScenePrompt`.** Step 2: dispatch via `veniceFetch()` in `src/services/veniceClient.ts` (routing through Electron IPC or Express proxy according to `desktopBridge`). Step 3: register asset. Returns a discriminated `SceneGenerationOutcome`. |
